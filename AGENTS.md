...
### 6. Git 冲突解决铁律（严禁直接使用 ours/theirs）
* 拉取更新或变基（`git pull --rebase`）发生代码冲突时，**绝对严禁直接使用 `--ours` 或 `--theirs` 无脑覆盖**（严禁粗暴丢失任何一方代码）。
* **必须仔细对比两方的 diff 修改与完整代码上下文**，逐行融合双方意图与逻辑，确保既不丢失远端的最新改动，又完整保留本地提交的修复与功能。
