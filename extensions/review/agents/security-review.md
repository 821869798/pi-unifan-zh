...
- 💡 **免检说明**：游戏或应用中的内部 GM（Game Master）指令面板、开发者调试控制台（Debug Console）及模拟工具属于受信任的开发辅助代码，在通过编译开关（如 `#if UNITY_EDITOR || DEVELOPMENT_BUILD`）或环境判断已与正式发布包隔离的前提下，严禁误判为安全隐患；但若此类入口在正式生产包中无防护暴露，仍属严重缺陷。
