...
const BASE_BRANCH_PROMPT_WITH_MERGE_BASE =
	"请审查当前分支相对于基准分支 '{baseBranch}' 的改动（合并基准 commit: {mergeBaseSha}）。直接运行 `git diff {mergeBaseSha}` 查看改动并直接输出审查发现。严禁输出任何步骤清单。所有输出必须使用纯正中文。";

const BASE_BRANCH_PROMPT_FALLBACK =
	"请审查当前分支相对于基准分支 '{branch}' 的改动。直接使用 `git merge-base` 与 `git diff` 查看改动并直接输出审查发现。严禁输出任何步骤清单。所有输出必须使用纯正中文。";

const COMMIT_PROMPT_WITH_TITLE =
	'请审查提交 commit {sha} ("{title}") 引入的代码改动。直接运行 `git show {sha}` 查看差异并直接输出审查发现。严禁输出任何步骤清单。所有输出必须使用纯正中文。';

const COMMIT_PROMPT =
	"请审查提交 commit {sha} 引入的代码改动。直接运行 `git show {sha}` 查看差异并直接输出审查发现。严禁输出任何步骤清单。所有输出必须使用纯正中文。";

const PULL_REQUEST_PROMPT =
	'请审查 Pull Request #{prNumber} ("{title}") 相对于基准分支 \'{baseBranch}\' 的改动（合并基准 commit: {mergeBaseSha}）。直接运行 `git diff {mergeBaseSha}` 查看改动并直接输出审查发现。严禁输出任何步骤清单。所有输出必须使用纯正中文。';

const PULL_REQUEST_PROMPT_FALLBACK =
	'请审查 Pull Request #{prNumber} ("{title}") 相对于基准分支 \'{baseBranch}\' 的改动。直接运行 `git diff` 查看改动并直接输出审查发现。严禁输出任何步骤清单。所有输出必须使用纯正中文。';

const FOLDER_REVIEW_PROMPT =
	"请对以下目录/文件路径的代码进行快照审查：{paths}。直接读取这些文件并直接输出审查发现。严禁输出任何步骤清单。所有输出必须使用纯正中文。";

// 权威的中文代码审查准则 (基于 Codex 准则精炼与本土化)
const REVIEW_RUBRIC = `> 🚨【最高执行原则】：
> 1. **全流程纯中文**：思维链（Thinking）、推理分析与最终报告必须 100% 使用纯正中文，严禁包含任何英文段落或英文小标题！
> 2. **直接审查，拒绝繁琐步骤**：严禁打印任何工作流待办清单（Checklist）或环境测试命令！直接阅读代码改动并立即输出审查报告！

# 核心代码审查准则

你是一名资深技术专家，正在审查提交的代码改动。请直击本质，拦截真实缺陷：

## 重点排查范围
1. **代码正确性与边界**：业务逻辑漏洞、空指针/未定义引用、边界越界、生命周期异常、未捕获的运行时崩溃。
2. **并发与状态安全**：竞态条件、死锁隐患、异步缺少等待、脏状态残留。
...
- ❌ 严禁基于纯主观假设进行无端猜测，每条问题必须有明确代码事实证据。
