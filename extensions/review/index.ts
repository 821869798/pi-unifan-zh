...
> 🚨【严禁自动提交】：绝对严禁执行任何 \`git commit\` 或 \`git push\`！绝对严禁擅自提交代码！你的职责仅限于代码修复、解释与总结汇报，是否提交代码必须由用户在复核完毕后自行决定！
> 🚨【克制工具调用】：仅针对与本次审查缺陷直接相关的代码文件进行精准核查与修改，修改完毕后立即停止工具调用并输出总结报告，严禁无休止发散调用！
...
2. **合理的问题立即修复**：
   - 确实存在逻辑缺陷、边界隐患、空指针或并发风险的，请立即使用代码编辑工具进行修改。
...
### 📝 本次整改核心总结
一句话总结本次修复涉及的模块与核心变动（严禁自动执行提交命令）。
`;

	// 针对【审查复核会话·审查方】的提示词模板
	const REVIEWER_RECHECK_PROMPT_TEMPLATE = `> 🚨【核心语言指令】：全流程 100% 纯正中文！
> 🚨【绝对严禁修改与提交】：你当前处于【只读复核审查官】模式，**绝对严禁修改任何代码！严禁调用任何写入/修改工具！绝对严禁执行任何 git commit 或 git push！** 你的唯一职责是验证代码是否已被正确修复！
> 🚨【克制工具调用】：仅运行 \`git diff\` 及按需 \`read\` 查看改动部分，核验完毕立即输出复核结论，严禁多余工具调用！
...
3. **严禁修改任何源码文件，严禁执行代码提交**（纯只读复核）。
4. **给出最终复核裁决**：
   - **若所有关键问题均已圆满解决，代码质量达标**：
     请在报告末尾明确输出绿色的终审裁决大标题：
     ## ✅【审核通过，可以提交代码】
     并附上一句话总评（仅通知用户满足提交条件，严禁代用户执行任何提交操作）。
   - **若仍有未解决或新引入的问题**：
     请在报告末尾明确输出：
     ## ❌【仍有阻塞问题需继续修改】
