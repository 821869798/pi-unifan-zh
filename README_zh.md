...
- **架构升级**：
  ...
  - **原生会话分支隔离技术**：彻底告别脆弱的 6 子代理并发通信、网络断流与超时风险，支持任意模型（GPT/Claude/DeepSeek/免费模型）。
- **核心审查命令**：
  - **`/review`** 🔍 **（交互式全功能审查）**：唤起全中文 TUI 选择弹窗，支持 6 种审查模式：
  ...
  - **`/review-lite`** ⭐ **（极速单兵审查）**：无需弹窗，直接对当前工作区未提交改动做闪电审查。
  - **`/end-review`** 🏁 **（结束审查并返回）**：
