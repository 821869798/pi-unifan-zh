...
- **架构亮点**：
  ...
  - **会话模型与思考档位继承**：单模型审查和主审汇总直接使用当前会话；多专家审查通过一个异步工作流启动，在每个专家请求中显式传入当前会话的 `提供方/模型:思考档位`，避免被子代理默认配置覆盖。
- **核心审查命令**：
  ...
  - **`/review-sync`** 🔄 **（双会话审查接力闭环 · 自动识别角色）**：
    - **开发会话粘贴审查意见**：自动识别为【代码修复】，逐项核验：有道理的立即改代码并总结修改了啥，没有道理的在总结中解释为什么没道理。
    - **审查会话粘贴修复报告**：自动识别为【只读复核】，绝不盲信文字报告，必须对照 `git diff` 实际看改动代码且**严禁改动任何代码**，满意后输出 `✅【审核通过，可以提交代码】`！
  - **`/review-end`** 🏁 **（结束审查并返回，别名 `/end-review`）**：
...
- **命令**：
  - **`/commit`**：自动分析暂存区/工作区代码变动（`git diff`），使用当前会话模型与思考档位，由提供方统一转换模型参数，根据实际暂存内容生成标准的 Conventional Commits 纯中文提交信息后提交。模型认证、请求、空结果或输出校验失败时会显示原因并停止提交与推送，保留暂存内容，不再使用固定兜底文案。
